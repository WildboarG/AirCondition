    ...
    while (1) {
        ...
        switch (state_s) {
            ...
            case STATE_WIFI_CONNECTED_OK://WiFi 连接成功
            {
                ...
                mqtt_start_connect(MQ_DEF_HOST, MQ_DEF_PORT, MQ_USERNAME, "test");
                //保存WiFi
 ...
 * @param state
*/
void user_state_send_notify(user_state_t state, int is_irq) // 用户状态通知
{
    if (stateTaskHandle==NULL) { // 没有创建
        LOG_E("\"state_task\" not create");
        return;
    }
    BaseType_t xHigherPriorityTaskWoken;  // 优先级
    xHigherPriorityTaskWoken = pdFALSE;  
    if (is_irq) // 中断
        xTaskNotifyFromISR(stateTaskHandle, state, eSetValueWithoutOverwrite, &xHigherPriorityTaskWoken);
        // 参数分别是 任务句柄, 通知内容, 通知内容类型, 优先级
    else
...
void going(int massage){
    ...
    if((massage<31) &&  (massage > 15)){
            ctlAC(massage); //设置空调温度
            return;
    }else{
       LOG_E("\"error code\" not exist"); 
    }
}
/**
